...
%    \includeonlyframes{current}
    ...
    \institute{\!\includegraphics[width=10em]{logo_EN_basic.png}}
...
%%     \usebeamertemplate*{title separator}
    ...
    \begin{frame}[label=current]{Введение: Состояния гонок}
        ...
        \note{
            Здесь представлен классический пример гонки на языке C++. Два потока пытаются увеличить переменную counter,
            если она меньше 1. Если выполнение \textbf{программы} будет спланировано \textbf{неудачно}, то обе проверки будут проведены
            \textbf{раньше}, чем оба увеличения, и в итоге значение counter будет равно 2.
        }
    ...
    \begin{frame}[label=current]{Введение: Состояния гонки в схемах сборки}
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                \only<1> \node[text width=3cm] at (9.5,1.1) {
                    \textit{\texttt{app} зависит от \texttt{lib.a}, но \textbf{не дожидается его}}
                };
                ...
                \mode<beamer>{
                    ...
                    \only<2> \node[red, style=draw, minimum width=2cm] (failure) at (9.75,0) {ошибка};

                    \only<2> \node[red, style=draw, dashed, minimum width=2cm] (error) at (4,0.75) {lib.a не готово};
                    ...
                    \only<2> \draw[red, line width=1pt, shorten >=2pt, shorten <=2pt, ->] (app) -- (failure);
                    ...
                    \only<3> \node[text width=3cm] at (7.8,0.55) {
                        \small{\textit{поток 2 ожидает \texttt{lib.a} перед связыванием приложения \texttt{app}}}
                    };
                ...
                % Common
                \node[minimum width=13cm, minimum height=3.5cm] (sizer) at (5.5,0.3) {};
                ...
                % Time text and arrow
                \node[gray, font=\itshape] (time) at (5.5, -1.0) {время};
        ...
        \pause
        \begin{itemize}
            \item Что если сборка библиотеки займет больше времени?
            \pause
            \item \textbf{Решение:} Определить пропущенную зависимость и добавить ее в схему сборки.
        \end{itemize}

        \note{
            Это был пример на C++, а теперь давайте поговорим о том, как состояния гонок могут возникать при сборке
            проектов.

            На слайде изображен процесс сборки некоторого проекта. В нем \textbf{библиотека} "lib" может собираться
            параллельно с \textbf{приложением}, которое ее использует.

            Однако в схеме сборки не указано, что для связывания приложения необходимо дождаться сборки библиотеки, и здесь
            \textbf{разработчику повезло}, и библиотека сама собой успела собраться к моменту связывания. Но если
            разработчику повезет \textbf{меньше} и сборка библиотеки \textbf{затянется}, то сборка всего проекта
            завершится неудачей.

            В этом случае разработчик вынужден либо перезапустить сборку и надеяться на лучшее, либо найти недостающую
            зависимость и добавить ее в схему сборки.
        }
    \end{frame}

    \begin{frame}[label=current]{Введение: Состояния гонок в реальных проектах}

        Такие проблемы иногда встречаются даже в крупных проектах.
        Они могут быть подтверждены следующими сообщениями об ошибках:
        ...
        \note{
            Состояния гонок коварны тем, что являются скрытой проблемой, которая может проявиться самым нежелательным
            образом. Спонтанная ошибка сборки - даже не самое страшное ее проявление. Например, состояния гонок могут
            приводить к уязвимостям и другим проблемам в собранном проекте. Позже мы рассмотрим такой пример.

            Связи с их скрытостью, их сложно искать людям. Даже в таких проектах, как Vim, над которым трудятся тысячи
            разработчиков, такие гонки присутствуют и остаются незамеченными.
        }
    \end{frame}
